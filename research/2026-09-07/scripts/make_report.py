...
def main():
    s=json.loads((ROOT/'results/final_summary.json').read_text());h=json.loads((ROOT/'results/history_summary.json').read_text());c=json.loads((ROOT/'results/current_summary.json').read_text())
    def pct(v):return '—' if v is None else f'{v*100:.1f}%'
    lines=[]
    for name,d in s['scenarios'].items():
        if 'h60m' in name and ('size' in name or name.endswith('l60s')):
            lines.append(f"| `{name}` | {d['priced']}/{d['eligible']} | {d['censored_count']} | {pct(d['conditional_median_return'])} |")
    status='\n'.join(f"| `{k}` | {v} |" for k,v in s['primary']['status_counts'].items())
    text=f'''# MemeHunter 样本研究、双模型评审与回测原型

研究日期：2026-09-07。最终数据汇总时间：{s['generated_at']}。
...
- 当前市场：抓取并整理 **{c['rows']}行网页观察样本**（新池20、股票配对20、热门池20）。已解析 **{c['unique_identified_pools']}个不同池标识**；{c['unresolved_pool_identity_rows']}行因页面失败/动态链接变化未确定池身份。不是60个独立币，也不是同一时间/区块截面。
- 历史：从本地Git逐次提取 **{h['snapshots']}份feed、{h['alert_events']}条去重告警、{h['observations']}条观测、{h['unique_pools']}个池、{h['unique_tokens']}个token**。时间范围7月10日至8月8日。保存原始JSON、提交时间、生成时间、首次可见时间与SHA-256。
- 模型：真实调用 **MiniMax M3、DeepSeek V4 Pro**，各一轮独立评审加一轮交叉复核，原文与调用元数据可审计。见 [评审裁决](reviews/ADJUDICATION.md)。模型文中的旧覆盖率、错误数字与公式不作为结论。
- 回测：已实现并测试下一bar执行、成本/延迟/规模敏感性、止盈止损双触发、缺失与未成交分类。**48个预先选定历史样本，已获得 {s['downloaded_selected_files']} 份行情文件、{s['downloaded_candle_rows']}根分钟bar**。这是OHLCV成交代理研究，缺历史真实深度、路线与已验证税费。
...
3. **发布稀疏**：本地保存的feed生成间隔中位数 **{h['median_cycle_gap_min']:.1f}分钟**，最长 **{h['max_cycle_gap_min']/60:.2f}小时**；只能证明所保存feed的节奏，不能断言每次云端任务是否执行。只有 **40个池** 有两条以上观测；旧feed没有存拒绝候选及完整V5数据。
...
**能完成价格代理回放：{s['primary']['priced']}/{s['primary']['eligible']}（{pct(s['primary']['coverage'])}）。** 缺失和删失仍留在48的分母里，未把它们当归零，也未把少数可定价结果外推为整体胜率。容量拒绝表示该假设规则下不入场，不能推断实际市场绝对无法成交。
